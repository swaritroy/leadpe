...
## Goal
Migrate everything from `leadpe.online` → `leadpe.online`, and enable real Vercel custom-domain attachment so live sites deploy at `{slug}.leadpe.online`.

## Scope: 33 files contain `leadpe.online` (353 references)

### 1. Domain string replacement (global find-replace `leadpe.online` → `leadpe.online`)
...
- `supabase/functions/verify-otp/index.ts` & all other functions: CORS allowlist regex `\.leadpe\.tech$` → `\.leadpe\.online$`, plus the literal `https://leadpe.online` origin
...
**Email/support:** keep `support@leadpe.online` in About/Privacy or change to `support@leadpe.online`?
...
```
Keep `leadpe.online` entries during transition? **Decision needed (see Q1).**
...
**Q1.** Keep `leadpe.online` as an active alias during transition (CORS + redirect), or kill it completely?

**Q2.** Change support email `support@leadpe.online` → `support@leadpe.online`?
...
## Out of scope
- Lovable platform's own `leadpe.online` custom domain — you must update that yourself in Lovable Project Settings → Domains (connect `leadpe.online`, set as primary, remove `.tech`).
