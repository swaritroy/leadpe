              ...
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                ...
                <div className="flex gap-4">
                  ...
                  <Button onClick={handleSubmit} disabled={loading} className="flex-1 h-12 rounded-xl text-white font-semibold" style={{ backgroundColor: "#00C853" }}>
                    {loading ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Processing...</span> : plan === "growth" ? `Pay ₹${MONTHLY_PRICE} & Go Live →` : "Launch My AI Business Site →"}
                  </Button>
                </div>
              </motion.div>
            )}
