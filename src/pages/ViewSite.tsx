    ...
    <div className="min-h-screen" style={{ fontFamily: `${font}, system-ui, sans-serif` }}>
      ...
      {isSuspended && (
        <div className="bg-destructive text-destructive-foreground px-4 py-3 text-center text-sm font-semibold flex items-center justify-center gap-2">
          ...
          Site Suspended — This business is currently inactive. Contact support.
        </div>
      )}

      {/* Nav */}
      <nav className="px-6 py-4 flex items-center justify-between" style={{ backgroundColor: primary, opacity: isSuspended ? 0.6 : 1 }}>
          ...
          <div className="container max-w-lg">
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmitLead} className="space-y-4">
                <Input required placeholder="Your Name" value={leadForm.name} onChange={e => setLeadForm({ ...leadForm, name: e.target.value })} />
                <Input type="email" placeholder="Email Address" value={leadForm.email} onChange={e => setLeadForm({ ...leadForm, email: e.target.value })} />
                <Input placeholder="Phone Number" value={leadForm.phone} onChange={e => setLeadForm({ ...leadForm, phone: e.target.value })} />
                <Textarea placeholder="How can we help?" value={leadForm.message} onChange={e => setLeadForm({ ...leadForm, message: e.target.value })} />
