// Sends formatted WhatsApp/SMS to admin (9973383902) via Twilio gateway,
...
async function sendTwilioWA(body: string): Promise<{ ok: boolean; sid?: string; err?: string }> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1) Send to admin via Twilio
    const adminBody = fmt(event_type, payload);
    const sent = await sendTwilioWA(adminBody);
    ...
    return new Response(
      JSON.stringify({ ok: true, admin_sent: sent.ok, queued_id, error: sent.err }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (e) {
